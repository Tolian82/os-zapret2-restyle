...
OUTPUT="${1:-/tmp/strategy-lab-blob-common-set-scaling.json}"
TRIALS="${2:-12}"

case "${TRIALS}" in ''|*[!0-9]*) echo "ERROR: trials must be an integer" >&2; exit 64 ;; esac
[ "${TRIALS}" -ge 4 ] 2>/dev/null && [ "${TRIALS}" -le 16 ] 2>/dev/null || {
    echo "ERROR: trials must be a multiple of 4 between 4 and 16" >&2
    exit 64
}
[ $((TRIALS % 4)) -eq 0 ] || {
    echo "ERROR: trials must be a multiple of 4 between 4 and 16" >&2
