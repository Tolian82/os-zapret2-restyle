...
# Lifecycle-lock owner for the experiment-only batched exhaustive Model-B benchmark.
# The benchmark reuses only the existing dedicated Model-B ports/rules and restores the
# exact normal-service state after the complete corpus replay.
...
[ -n "${OUTPUT}" ] || { echo 'ERROR: exhaustive Model B output path is required' >&2; exit 64; }
...
[ "${STRATEGY_LAB_LIFECYCLE_OWNER:-0}" = 1 ] || { echo 'ERROR: exhaustive Model B has no lifecycle-lock owner' >&2; exit 77; }
( : >&9 ) 2>/dev/null || { echo 'ERROR: exhaustive Model B lifecycle lock descriptor is unavailable' >&2; exit 77; }
[ -x "${STRATEGY_LAB_SERVICE_SCRIPT:-}" ] || { echo 'ERROR: exhaustive Model B service control is unavailable' >&2; exit 70; }

SESSION_ROOT="${STRATEGY_LAB_RUN_DIR}/model-b-exhaustive"
...
# dvtws2 drops to nobody and must retain search-only traversal to job-owned hostlists.
...
finalize_report()
{
    ...
    if [ -f "${OUTPUT}" ] && [ -f "${INITIAL_EVIDENCE}" ] && [ -f "${FINAL_EVIDENCE}" ]; then
        "${PYTHON_LAUNCHER}" model-b-exhaustive finalize "${OUTPUT}" "${INITIAL_EVIDENCE}" "${FINAL_EVIDENCE}" "${cleanup_ok}" >/dev/null 2>&1 || true
    fi
}
...
capture_evidence "${INITIAL_EVIDENCE}" || { echo 'ERROR: exhaustive Model B could not capture initial Zapret2 evidence' >&2; exit 70; }
initial_state=$("${STRATEGY_LAB_JQ}" -r '.state' "${INITIAL_EVIDENCE}")

[ -r "${REFERENCE_STATUS}" ] || { echo 'ERROR: exhaustive Model B reference status is unavailable' >&2; exit 70; }
...
[ "${reference_initial}" = "${initial_state}" ] || {
    echo "ERROR: current Zapret2 state ${initial_state} does not match exhaustive reference ${reference_initial}" >&2
    exit 65
}

"${MODEL_B_ADAPTER}" cleanup-all >/dev/null 2>&1 || {
    echo 'ERROR: exhaustive Model B could not clear dedicated stale residue' >&2
    exit 70
}
"${MODEL_B_ADAPTER}" preflight >/dev/null 2>&1 || {
    echo 'ERROR: exhaustive Model B dedicated ports/rules are not available' >&2
    exit 70
}
...
case "${initial_state}" in
    RUNNING)
        "${STRATEGY_LAB_SERVICE_SCRIPT}" strategy-lab-stop >/dev/null 2>&1 || {
            echo 'ERROR: exhaustive Model B could not stop normal Zapret2 service' >&2
            exit 70
        }
        service_changed=1
        if "${STRATEGY_LAB_SERVICE_SCRIPT}" strategy-lab-status >/dev/null 2>&1; then
...
"${PYTHON_LAUNCHER}" model-b-exhaustive run "${REFERENCE_JOB}" "${OUTPUT}"
python_status=$?
...
printf 'Model B exhaustive report: %s\n' "${OUTPUT}"
printf 'Model B exhaustive conclusion: %s\n' "${conclusion}"
exit 0
