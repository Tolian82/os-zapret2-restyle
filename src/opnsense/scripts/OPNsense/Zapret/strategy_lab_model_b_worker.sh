...
# Lifecycle-lock owner for the experiment-only Model-B coexistence run.  The caller is
# zapret_service.sh strategy-lab-model-b, which holds FD 9 for the complete experiment.
# This wrapper guarantees exact normal-service restoration after the Python measurement.
...
[ -n "${OUTPUT}" ] || { echo 'ERROR: Model B output path is required' >&2; exit 64; }
[ -x "${PYTHON_LAUNCHER}" ] || { echo 'ERROR: Strategy Lab Python launcher is unavailable' >&2; exit 70; }
[ -x "${MODEL_B_ADAPTER}" ] || { echo 'ERROR: Model B system adapter is unavailable' >&2; exit 70; }
[ -x "${STRATEGY_LAB_JQ}" ] || { echo 'ERROR: jq is unavailable' >&2; exit 70; }
[ "${STRATEGY_LAB_LIFECYCLE_OWNER:-0}" = 1 ] || { echo 'ERROR: Model B has no lifecycle-lock owner' >&2; exit 77; }
( : >&9 ) 2>/dev/null || { echo 'ERROR: Model B lifecycle lock descriptor is unavailable' >&2; exit 77; }
[ -x "${STRATEGY_LAB_SERVICE_SCRIPT:-}" ] || { echo 'ERROR: Model B service control is unavailable' >&2; exit 70; }

SESSION_ROOT="${STRATEGY_LAB_RUN_DIR}/model-b"
...
finalize_report()
{
    ...
    if [ -f "${OUTPUT}" ] && [ -f "${INITIAL_EVIDENCE}" ] && [ -f "${FINAL_EVIDENCE}" ]; then
        "${PYTHON_LAUNCHER}" model-b finalize "${OUTPUT}" "${INITIAL_EVIDENCE}" "${FINAL_EVIDENCE}" "${cleanup_ok}" >/dev/null 2>&1 || true
    fi
}
...
on_exit()
{
    ...
    if "${MODEL_B_ADAPTER}" cleanup-all >/dev/null 2>&1; then
        cleanup_ok=1
    else
        cleanup_ok=0
    fi
...
capture_evidence "${INITIAL_EVIDENCE}" || { echo 'ERROR: Model B could not capture initial Zapret2 evidence' >&2; exit 70; }
initial_state=$("${STRATEGY_LAB_JQ}" -r '.state' "${INITIAL_EVIDENCE}")

[ -r "${REFERENCE_STATUS}" ] || { echo 'ERROR: Model B reference status is unavailable' >&2; exit 70; }
...
[ "${reference_initial}" = "${initial_state}" ] || {
    echo "ERROR: current Zapret2 state ${initial_state} does not match Model A reference ${reference_initial}" >&2
    exit 65
}

# Dedicated Model-B residue is safe to clean because the lifecycle lock excludes every
# normal Strategy Lab owner and the adapter can touch only rules 19128-19130 / ports
# 9990-9992 with verified dvtws2 identity.
"${MODEL_B_ADAPTER}" cleanup-all >/dev/null 2>&1 || {
    echo 'ERROR: Model B could not clear its dedicated stale residue' >&2
    exit 70
}
"${MODEL_B_ADAPTER}" preflight >/dev/null 2>&1 || {
    echo 'ERROR: Model B dedicated ports/rules are not available' >&2
    exit 70
}
...
case "${initial_state}" in
    RUNNING)
        "${STRATEGY_LAB_SERVICE_SCRIPT}" strategy-lab-stop >/dev/null 2>&1 || {
            echo 'ERROR: Model B could not stop normal Zapret2 service' >&2
            exit 70
        }
        service_changed=1
        if "${STRATEGY_LAB_SERVICE_SCRIPT}" strategy-lab-status >/dev/null 2>&1; then
...
"${PYTHON_LAUNCHER}" model-b run "${REFERENCE_JOB}" "${OUTPUT}"
python_status=$?
...
printf 'Model B report: %s\n' "${OUTPUT}"
printf 'Model B conclusion: %s\n' "${conclusion}"
exit 0
