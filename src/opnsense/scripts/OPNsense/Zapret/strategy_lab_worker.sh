...
STRATEGY_LAB_STAGE80_TIMEOUT="${STRATEGY_LAB_STAGE80_TIMEOUT:-60}"
set -eu
umask 022
for module in common state firewall runtime candidate lifecycle target request result probe family expansion stability extended
do
    path="${MODULE_DIR}/${module}.sh"; [ -r "${path}" ] || { echo "ERROR: required Strategy Lab module is missing: ${path}" >&2; exit 1; }; . "${path}"
done
JOB_ID="${1:-}"
strategy_lab_job_id_valid "${JOB_ID}" || exit 64
strategy_lab_require_jq
for runner in "${PROBE_RUNNER}" "${CANDIDATE_RUNNER}" "${EXPANSION_RUNNER}" "${STABILITY_RUNNER}" "${EXTENDED_RUNNER}"; do [ -x "${runner}" ] || exit 1; done
STATUS_FILE=$(strategy_lab_status_file "${JOB_ID}"); CANCEL_FILE=$(strategy_lab_cancel_file "${JOB_ID}"); JOB_DIR=$(strategy_lab_job_dir "${JOB_ID}")
...
for worker_module in worker_messages worker_control worker_expansion_messages worker_stability_messages worker_extended_messages worker_flow
do
    path="${MODULE_DIR}/${worker_module}.sh"; [ -r "${path}" ] || exit 1; . "${path}"
done
