...
worker_budget_initialize()
{
    ...
    do
        case "${_wb_value}" in ''|*[!0-9]*|0) return 1 ;; esac
    done
...
worker_budget_begin_stage80()
{
    ...
    WORKER_STAGE80_DEADLINE_EPOCH=$(worker_budget_min "${_wb_stage_deadline}" "${WORKER_OVERALL_DEADLINE_EPOCH}") || return 1
...
{
    case "$1" in
        80) [ -n "${WORKER_STAGE80_DEADLINE_EPOCH:-}" ] || return 1; printf '%s\n' "${WORKER_STAGE80_DEADLINE_EPOCH}" ;;
...
worker_budget_timeout_for()
{
    ...
    case "${_wb_operation_limit}" in ''|*[!0-9]*|0) return 1 ;; esac
