...
strategy_lab_lifecycle_snapshot_file()
{
    printf '%s/lifecycle-snapshot.json\n' "${JOB_DIR}"
}

strategy_lab_lifecycle_restoration_file()
{
    printf '%s/lifecycle-restoration.json\n' "${JOB_DIR}"
}
...
strategy_lab_record_json_field()
{
    ...
    "${STRATEGY_LAB_JQ}" --arg field "${_strategy_lab_record_field}" \
        --slurpfile value "${_strategy_lab_record_file}" \
...
{
    _strategy_lab_evidence_output="$1"
    _strategy_lab_evidence_tmp="${_strategy_lab_evidence_output}.tmp.$$"
    if "${STRATEGY_LAB_SERVICE_SCRIPT}" strategy-lab-evidence > "${_strategy_lab_evidence_tmp}" 2>/dev/null; then
        _strategy_lab_evidence_status=0
    else
        _strategy_lab_evidence_status=$?
    fi
    if [ "${_strategy_lab_evidence_status}" -eq 64 ]; then
        rm -f "${_strategy_lab_evidence_tmp}"
        return 2
    fi
    [ "${_strategy_lab_evidence_status}" -eq 0 ] || {
        rm -f "${_strategy_lab_evidence_tmp}"
        return 1
    }
    "${STRATEGY_LAB_JQ}" -e '
...
{
    _strategy_lab_snapshot="$1"
    _strategy_lab_expected_state="$2"
    "${STRATEGY_LAB_JQ}" -e --arg state "${_strategy_lab_expected_state}" '
        .state==$state and
        .effective_config_hash!="missing" and
        (if $state=="RUNNING" then
            .child_running==true and
            .supervisor_running==true and
            .runtime_args_hash!="missing" and
            .normal_firewall_hash!="empty" and
            .normal_firewall_hash!="missing"
         else
            .child_running==false and
            .supervisor_running==false and
            .normal_firewall_hash=="empty"
         end)
...
strategy_lab_write_legacy_snapshot()
{
    ...
    "${STRATEGY_LAB_JQ}" -nc --arg state "${STRATEGY_LAB_INITIAL_SERVICE_STATE}" '
        {schema:1,source:"legacy-status",state:$state,child_running:null,
         supervisor_running:null,runtime_args_hash:"unavailable",
         effective_config_hash:"unavailable",normal_firewall_hash:"unavailable"}
...
strategy_lab_capture_initial_service_state()
{
    if strategy_lab_service_status_code; then
        ...
        case "${_strategy_lab_state_status}" in
            1) STRATEGY_LAB_INITIAL_SERVICE_STATE="STOPPED" ;;
            *)
    ...
    if strategy_lab_fetch_semantic_evidence "${_strategy_lab_snapshot}"; then
        ...
        strategy_lab_semantic_snapshot_valid "${_strategy_lab_snapshot}" \
            "${STRATEGY_LAB_INITIAL_SERVICE_STATE}" || {
                STRATEGY_LAB_INITIAL_SERVICE_STATE="INCOMPLETE"
                return 1
            }
        ...
        _strategy_lab_evidence_status=$?
        [ "${_strategy_lab_evidence_status}" -eq 2 ] || {
            STRATEGY_LAB_INITIAL_SERVICE_STATE="INCOMPLETE"
            return 1
        }
...
strategy_lab_cleanup_temporary_runtime()
{
    ...
    if command -v strategy_lab_firewall_range_empty >/dev/null 2>&1; then
        strategy_lab_firewall_range_empty || return 1
    fi
    if command -v strategy_lab_candidate_pid_file >/dev/null 2>&1 &&
        command -v strategy_lab_candidate_process_running >/dev/null 2>&1 &&
        [ -n "${JOB_ID:-}" ]; then
...
{
    _strategy_lab_restore_verified="$1"
    _strategy_lab_restore_source="$2"
    _strategy_lab_restore_final_state="$3"
    _strategy_lab_restore_strategy_unchanged="$4"
    _strategy_lab_restore_runtime_clean="$5"
    _strategy_lab_restore_output=$(strategy_lab_lifecycle_restoration_file)
    "${STRATEGY_LAB_JQ}" -nc \
        --argjson verified "${_strategy_lab_restore_verified}" \
        --arg source "${_strategy_lab_restore_source}" \
        --arg initial_state "${STRATEGY_LAB_INITIAL_SERVICE_STATE}" \
        --arg final_state "${_strategy_lab_restore_final_state}" \
        --argjson strategy_unchanged "${_strategy_lab_restore_strategy_unchanged}" \
        --argjson temporary_runtime_clean "${_strategy_lab_restore_runtime_clean}" \
        '{verified:$verified,source:$source,initial_state:$initial_state,
          final_state:$final_state,strategy_unchanged:$strategy_unchanged,
          temporary_runtime_clean:$temporary_runtime_clean}' \
        > "${_strategy_lab_restore_output}" || return 1
...
{
    _strategy_lab_initial=$(strategy_lab_lifecycle_snapshot_file)
    _strategy_lab_final="${JOB_DIR}/lifecycle-final.json"
    strategy_lab_fetch_semantic_evidence "${_strategy_lab_final}" || return 1
    strategy_lab_semantic_snapshot_valid "${_strategy_lab_final}" \
        "${STRATEGY_LAB_INITIAL_SERVICE_STATE}" || return 1

    _strategy_lab_strategy_unchanged=false
    _strategy_lab_runtime_clean=true
    if "${STRATEGY_LAB_JQ}" -e --slurpfile initial "${_strategy_lab_initial}" '
        .effective_config_hash==$initial[0].effective_config_hash and
        .runtime_args_hash==$initial[0].runtime_args_hash and
        .normal_firewall_hash==$initial[0].normal_firewall_hash
    ' "${_strategy_lab_final}" >/dev/null; then
        _strategy_lab_strategy_unchanged=true
    fi
    [ "${_strategy_lab_strategy_unchanged}" = true ] || {
        strategy_lab_write_restoration_result false zapret_service \
            "${STRATEGY_LAB_INITIAL_SERVICE_STATE}" false true || true
        return 1
    }
    strategy_lab_write_restoration_result true zapret_service \
        "${STRATEGY_LAB_INITIAL_SERVICE_STATE}" true "${_strategy_lab_runtime_clean}"
}

strategy_lab_restore_initial_service_state()
{
    if ! strategy_lab_cleanup_temporary_runtime; then
        strategy_lab_write_restoration_result false \
            "${STRATEGY_LAB_INITIAL_EVIDENCE_SOURCE:-unknown}" unknown false false || true
        return 1
    fi
    ...
    case "${STRATEGY_LAB_INITIAL_SERVICE_STATE}" in
        RUNNING)
            strategy_lab_restore_running_state || {
                strategy_lab_write_restoration_result false \
                    "${STRATEGY_LAB_INITIAL_EVIDENCE_SOURCE:-unknown}" unknown false true || true
                return 1
            }
            ;;
        STOPPED)
    ...
    if [ "${STRATEGY_LAB_INITIAL_EVIDENCE_SOURCE}" = zapret_service ]; then
        strategy_lab_verify_semantic_restoration
        return $?
    fi

    strategy_lab_write_restoration_result true legacy-status \
        "${STRATEGY_LAB_INITIAL_SERVICE_STATE}" true true
}
