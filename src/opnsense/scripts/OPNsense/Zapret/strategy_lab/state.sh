...
strategy_lab_state_lock_acquire()
{
    if [ -x "${STRATEGY_LAB_STATE_LOCKF_BIN}" ]; then
        "${STRATEGY_LAB_STATE_LOCKF_BIN}" -s -t "${STRATEGY_LAB_STATE_LOCK_TIMEOUT}" 9
        return $?
    fi
    if [ -n "${STRATEGY_LAB_STATE_FLOCK_BIN}" ] && [ -x "${STRATEGY_LAB_STATE_FLOCK_BIN}" ]; then
        "${STRATEGY_LAB_STATE_FLOCK_BIN}" -x -w "${STRATEGY_LAB_STATE_LOCK_TIMEOUT}" 9
        return $?
    fi
    return 1
}
...
strategy_lab_state_transform()
{
    ...
    (
        ...
        _strategy_lab_transform_tmp=$(mktemp "$(dirname "${_strategy_lab_transform_status}")/.state.XXXXXX") || exit 1
        "${STRATEGY_LAB_JQ}" "$@" "(${_strategy_lab_transform_filter}) | .revision=((.revision // 0)+1)" \
...
strategy_lab_initialize_state()
{
    ...
    "${STRATEGY_LAB_JQ}" -nc \
        --arg job_id "${_strategy_lab_job}" --arg target "${_strategy_lab_target}" \
        --arg mode "${_strategy_lab_mode}" --arg language "${_strategy_lab_language}" '
        {
            schema:2,revision:0,job_id:$job_id,state:"queued",outcome:"",
            target:$target,target_type:"",endpoints:[],network:{},baseline:{},candidate_smoke:{},
            family_screening:{},parameter_expansion:{},stability:{},shortlist:{},extended:{},quic:{},udp:{},
            mode:$mode,language:$language,initial_service_state:"",cancel_requested:false,
            cancel_requested_at:"",current_stage:"00",message:"",
            progress:{percent:0,stage:"00",stage_key:"target_initialization",message:""},
            circular_eligible:false,circular_eligibility_reason:"not_completed",circular_candidate_count:0,
            stages:[
...
{
    _strategy_lab_endpoints_json=$("${STRATEGY_LAB_JQ}" -Rsc 'split("\n") | map(select(length > 0))' "$4") || return 1
    strategy_lab_state_transform "$1" '.target=$target | .target_type=$target_type | .endpoints=$endpoints' \
        --arg target "$2" --arg target_type "$3" --argjson endpoints "${_strategy_lab_endpoints_json}"
}
...
strategy_lab_set_candidate_smoke_result(){ strategy_lab_state_transform "$1" '.candidate_smoke=$candidate[0]' --slurpfile candidate "$2"; }
...
strategy_lab_request_cancel()
{
    ...
    ' --arg requested_at "${_strategy_lab_requested_at}" --arg message "$2"
}

strategy_lab_update_job()
{
    _strategy_lab_job="$1"; _strategy_lab_state="$2"; _strategy_lab_outcome="$3"
    _strategy_lab_stage="$4"; _strategy_lab_canceled="$5"; _strategy_lab_message="$6"
    strategy_lab_state_transform "${_strategy_lab_job}" '
        def pct($s): {"00":0,"10":9,"20":18,"30":27,"40":36,"50":45,"60":55,"70":64,"80":73,"85":82,"90":91,"99":100}[$s] // 0;
        if (.state=="completed" or .state=="error") then .
        else
            ...
            .progress.percent=pct($stage) | .progress.stage=$stage |
            .progress.stage_key=([.stages[]|select(.number==$stage)|.key][0] // "") |
            .progress.message=$message
        end
    ' --arg state "${_strategy_lab_state}" --arg outcome "${_strategy_lab_outcome}" \
      --arg stage "${_strategy_lab_stage}" --argjson canceled "${_strategy_lab_canceled}" \
      --arg message "${_strategy_lab_message}"
}

strategy_lab_set_initial_service_state()
{
    strategy_lab_state_transform "$1" '.initial_service_state=$service_state' --arg service_state "$2"
}

strategy_lab_update_stage()
{
    _strategy_lab_number=$(printf '%02d' "$2")
    strategy_lab_state_transform "$1" '
        def pct($s): {"00":0,"10":9,"20":18,"30":27,"40":36,"50":45,"60":55,"70":64,"80":73,"85":82,"90":91,"99":100}[$s] // 0;
        if (.state=="completed" or .state=="error") then .
        else
            ...
            .current_stage=$number | .progress.percent=pct($number) |
            .progress.stage=$number |
            .progress.stage_key=([.stages[]|select(.number==$number)|.key][0] // "") |
            .progress.message=$message
        end
    ' --arg number "${_strategy_lab_number}" --arg status "$3" --arg message "$4"
}
...
{
    _strategy_lab_stage=$(printf '%02d' "$2")
    _strategy_lab_events=$(strategy_lab_event_file "$1")
    _strategy_lab_lock=$(strategy_lab_state_lock_file "$1")
    (
        strategy_lab_state_lock_acquire || exit 75
        "${STRATEGY_LAB_JQ}" -nc --arg stage "${_strategy_lab_stage}" --arg status "$3" --arg message "$4" \
