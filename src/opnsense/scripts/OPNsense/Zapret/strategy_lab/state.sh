...
strategy_lab_set_target_contract()
{
    ...
    _strategy_lab_endpoints_json=$("${STRATEGY_LAB_JQ}" -Rsc \
        'split("\n") | map(select(length > 0))' \
        "${_strategy_lab_endpoints_file}") || return 1
...
{
    strategy_lab_state_transform "$1" '.network=$network[0]' \
        --slurpfile network "$2"
}

strategy_lab_set_baseline_result()
{
    strategy_lab_state_transform "$1" '.baseline=$baseline[0]' \
        --slurpfile baseline "$2"
}

strategy_lab_set_candidate_smoke_result()
{
    strategy_lab_state_transform "$1" '.candidate_smoke=$candidate[0]' \
...
strategy_lab_update_job()
{
    ...
    strategy_lab_state_transform "${_strategy_lab_job}" '
        ...
        if (.state=="completed" or .state=="error") then .
            ...
            .state=(
                if $existing_cancel and ($state=="queued" or $state=="running")
...
strategy_lab_append_event()
{
    ...
    _strategy_lab_message="$4"
    _strategy_lab_events=$(strategy_lab_event_file "${_strategy_lab_job}")
    _strategy_lab_lock=$(strategy_lab_state_lock_file "${_strategy_lab_job}")
