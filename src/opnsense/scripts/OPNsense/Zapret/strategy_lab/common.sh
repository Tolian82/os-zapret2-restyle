...
strategy_lab_pid_file()
{
    printf '%s/worker.pid\n' "$(strategy_lab_job_dir "$1")"
}

strategy_lab_cancel_file()
{
    printf '%s/cancel.request\n' "$(strategy_lab_job_dir "$1")"
}

strategy_lab_event_file()
{
    printf '%s/events.ndjson\n' "$(strategy_lab_job_dir "$1")"
}

strategy_lab_log_file()
{
...
strategy_lab_clear_active_job()
{
    ...
    if [ "${_strategy_lab_current}" = "${_strategy_lab_expected}" ]; then
...
strategy_lab_job_active()
{
    ...
    case "${_strategy_lab_state}" in
        queued|running|cancel_requested)
            strategy_lab_pid_running "${_strategy_lab_pidfile}"
            return $?
            ;;
    esac
    return 1
