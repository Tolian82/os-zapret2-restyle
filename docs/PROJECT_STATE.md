...
Current hardening source candidate: `os-zapret2-restyle-0.3.2_29.pkg`
...
- Patch `_29` — stale nonterminal job reconciliation after worker disappearance: current source candidate awaiting its own required GitHub checks;
- Patch `_30` and later work must not begin until `_29` is fully verified.
...
`VERSION=0.3.2`; `PLUGIN_REVISION=29`.

Next action: complete required GitHub checks and remote verification for Patch `_29`. Patch `_30` remains blocked until that succeeds.
