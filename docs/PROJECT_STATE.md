...
Current hardening source candidate: `os-zapret2-restyle-0.3.2_30.pkg`
...
- Patch `_28` — hard whole-worker deadline: required GitHub checks and FreeBSD package build completed successfully;
- Patch `_29` — stale nonterminal job reconciliation: required GitHub checks and FreeBSD package build completed successfully;
- Patch `_30` — per-job serialization of Strategy Lab state updates: current source candidate awaiting its own required GitHub checks;
- Patch `_31` and later work must not begin until `_30` is fully verified.
...
`VERSION=0.3.2`; `PLUGIN_REVISION=30`.

Next action: complete required GitHub checks and remote verification for Patch `_30`. Patch `_31` remains blocked until that succeeds.
