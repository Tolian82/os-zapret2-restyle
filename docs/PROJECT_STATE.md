...
- [reverse position-16 live result and guarded position-24 candidate](verification/evidence/2026-09-23-telegram-voice-reverse16.md).
...
- Guarded reverse position 24 is prepared as `tgvoice_ipfrag24_reverse_postnat_v1.py`, revision `postnat-reverse24-v1`, SHA-256 `c63ddb5dc097dfcec2759134101a31f1020cf5536d82f26eeab3dd2b30217905`. It retains the same bounded runner/lifecycle, uses native reverse fragmentation at UDP position 24, and passes application payloads of at most 16 bytes unchanged to avoid native unfragmented fallback. Python syntax passed; live position-24 evidence is pending.
...
Follow the exact next action in [`START_HERE.md`](START_HERE.md): run the prepared guarded reverse position-24 candidate with the same qualified binary/runtime and endpoint, then qualify wire/media/restoration evidence. Reverse positions 8, 16 and 32 are now locally wire-qualified; the earlier positive owner observation remains separately unqualified. If reverse24 is also wire-correct without reply/media, stop widening standalone fragment positions and move to the next bounded candidate family. Reach repeated `MEDIA_PASS` and a real remote Windows/Android `CALL_PASS`. There is no current independent working control; its absence limits causal `NETWORK_FAIL` attribution, not continued work. A failed restoration remains overriding `RESTORE_FAILED`.
