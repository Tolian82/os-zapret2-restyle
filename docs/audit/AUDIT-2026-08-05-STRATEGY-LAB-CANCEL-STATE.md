...
- serializes status mutations;
- persists `state=cancel_requested` and `cancel_requested=true`;
...
- prevents a late non-terminal job update from clearing the request.

## Automated evidence

`scripts/test-strategy-lab-cancel-state.sh` covers persisted response equality, repeated cancel, localization, concurrent stage updates, late worker updates, terminal immutability, valid JSON, and temporary-file cleanup.
