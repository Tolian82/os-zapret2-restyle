...
Historical `_28..._33` planning text is superseded by the implemented/current contract below. Implementation chronology remains in `docs/patches/`, `docs/devlog/` and verification evidence.
...
Shell/runtime adapters do not choose the next candidate, expand the search graph or silently invent strategy semantics.

## CandidateSpec

Each candidate is one immutable normalized identity containing enough information to reproduce the exact tested strategy, including as applicable:
...
`family` is diagnostic/ordering metadata, not authorization. Stage-50 family evidence may influence priority but must not turn the Stage-60 graph into an allowlist.
...
Presence of a resource means available, not required by every candidate. Search eligibility remains semantic and does not create Cartesian combinations from unrelated installed files.
...
Candidate functional dependencies are part of reproducibility; runtime preload policy is an execution optimization and must not alter CandidateSpec identity.
...
If the target already works directly under the required baseline contract, return `TARGET_ACCESSIBLE`; do not search for an unnecessary bypass.
...
Current production discovery remains the bounded 4 KiB GET selected after the `_5/_6` measurement cycle. HEAD/GET-1 did not justify a production change.

Stability/final result logic remains owned by Python/downstream stages and must preserve exact CandidateSpec, endpoint/resource identity and execution evidence. Runtime acceleration must not turn an infrastructure failure into a candidate PASS/FAIL result.

Historical finalist/deep-validation proposals are not automatic current requirements merely because they appeared in an old target-design sequence; current implemented behavior and current task docs control.

## Current runtime selection

The old A/B/C comparison is **closed as a selection question**.

- Model C: only normal production Stage-60 runtime since `v0.4.1_13`;
- Model B: retained explicit warm/reference/benchmark tooling;
- Model A: retained explicit cold correctness/reference tooling.

Normal production has no automatic `C -> B -> A` replay. A Model-C infrastructure failure is an explicit structural Stage-60 failure and remains distinct from candidate network evidence.
...
See `docs/architecture/STRATEGY_LAB_MODEL_C.md` for the current physical runtime contract and Model-C-only failure boundary.
...
Candidate/operation admission uses remaining absolute budget; cleanup/restoration is never made unbounded to gain more search time.

The current parent-budget policy is `eligible-work-v1`; see `docs/architecture/STRATEGY_LAB_ADAPTIVE_BUDGET.md`.

The previously observed containment question concerned the retired transitional `C -> B` replay path and did not justify preserving or optimizing that path. If Model-C-only operation later exposes a concrete timeout/deadline defect, investigate it as a separate evidence-based task.
...
`v0.4.1_13` finalizes normal production as Model-C-only without changing this search graph, CandidateSpec, resource, endpoint, discovery or result contract.

After `_13` automated qualification and the selected owner-live Model-C-only regression, return to the current `docs/ROADMAP.md` rather than reopening historical A/B/C selection or old `_32/_33` sequencing by default.
