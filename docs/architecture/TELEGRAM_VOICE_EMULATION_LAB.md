...
**Status:** CURRENT TEMPORARY DESIGN · REVERSE16 WIRE_OK / NO_REPLY_UNKNOWN · GUARDED REVERSE24 NEXT
**Updated:** 2026-09-23
...
4. guarded reverse position 32 and guarded reverse position 16 — both live-qualified on September 23 as `WIRE_OK / NO_REPLY_UNKNOWN / RESTORE_OK`, with no media pass; guarded reverse position 24 is the final planned standalone position comparison;
5. if reverse24 is also wire-correct and silent, stop widening standalone positions and move to a bounded fake-plus-fragment family;
6. stop widening if the control-proven endpoint remains silent for every on-wire-correct family.

Position 8 separates the UDP header from the complete 40-byte Hello. Position 32 cuts inside the constant reflector marker. Exact reverse position-8, position-32 and position-16 serialization are now live-qualified. For position 32 the pinned native Lua would fall back to unfragmented raw-send when UDP application payload length is at most 24 bytes; its wrapper returns `VERDICT_PASS` without raw-send for those short datagrams. Position 16 lowers that guard to 8 bytes. The live position-16 run confirmed 60/60 reverse pairs: IP length 52 at offset 16, MF=0, followed by IP length 36 at offset 0, MF=1; all intercepted application payloads were 40 bytes, so the short guard was not exercised. The prepared position-24 variant uses the same guard pattern with an application-payload threshold of 16 bytes. For the observed 40-byte Hello it should emit two equal IP length-44 fragments, offset 24/MF=0 first and offset 0/MF=1 second. Payloads of 17–24 bytes will fragment under position 24 although they passed under the position-32 guard, so do not describe every possible packet as a single-factor comparison. If position 24 is also wire-correct and silent, the standalone position sweep ends. Do not count pre-NAT or first-fragment-recapture defects as failed candidates for the corrected setup. Engine/custom-parameter comparisons belong to separate epochs so their effects are not mixed with fragmentation changes.
...
10. [ ] Live-test the prepared guarded reverse position-24 runner; if it is wire-correct and silent, end standalone position widening.
11. [ ] Investigate real-client engine/configuration parity in separate experiments where source evidence justifies it.
12. [ ] Reach and repeat `MEDIA_PASS` through OPNsense with both peers established, stats/BWE on both sides and exit 0.
13. [ ] Complete one remote P2P-disabled Windows/Android call with two-way sound and sustained bidirectional UDP (`CALL_PASS`).
14. [ ] Remove temporary owned routes/access/scripts at closeout; archive results and decide the paused `_4` branch.
