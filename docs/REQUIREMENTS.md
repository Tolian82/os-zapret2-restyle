...
- current source candidate: `0.4.0_5`, adaptive-search `_31`; latest published and
...
- the adaptive-search design is approved and partially implemented: `_28` removes the
  ...
  plus exact Python-owned runtime rendering, `_30` adds the native DAG, golden corpus,
  semantic resource branches and candidate-defined ranges, and `_31` adds live-evidence
  ordering, fixed endpoint epochs, three-winner defaults and timing telemetry; `_32`–`_33`
  remain pending;
