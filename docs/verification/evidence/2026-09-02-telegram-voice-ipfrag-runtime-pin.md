...
This is a runtime-capability result only. It does not show that the provider accepts IPv4 fragments, returns TURN/STUN, accepts Telegram Reflector Hello, or carries media over UDP.

On 2026-09-03 the owner selected an automatic Telegram-traffic emulator/oracle as the next boundary. The prepared remote branch `v0.5.0_4-telegram-voice-ipfrag` preserves the ordered STUN candidate, but it has no PR, exact-head CI, merged package or network result and is paused. This evidence continues to prove the primitive is available; it does not make that branch release-ready or supersede the current [`Telegram Voice emulation architecture`](../../architecture/TELEGRAM_VOICE_EMULATION_LAB.md).
