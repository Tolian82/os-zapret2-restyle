...
if grep -Eq 'FIRMWARE_REGISTER.*\|\|[[:space:]]*true|CONFIGURE_PLUGINS.*\|\|[[:space:]]*true' "${POST_HOOK}"; then
    fail "post-install suppresses OPNsense registration or configuration failure"
fi
