...
assert_cancelled "${status}" 'Late worker update'

(
    _iteration=0
    while [ "${_iteration}" -lt 20 ]
    do
        strategy_lab_update_stage "${job}" 70 RUNNING "tick-${_iteration}"
        _iteration=$((_iteration + 1))
    done
) &
writer_pid=$!
cancel_job cancel "${job}" > "${TEST_ROOT}/concurrent.json"
wait "${writer_pid}"
assert_cancelled "${status}" 'Cancellation requested'
"${STRATEGY_LAB_JQ}" -e . "${status}" >/dev/null || fail "concurrent updates damaged JSON"
...
if find "${STRATEGY_LAB_JOBS_DIR}" -type d -name '*.lock' | grep -q .; then
    fail "state lock directory was left behind"
fi
if find "${STRATEGY_LAB_JOBS_DIR}" -type f -name '.*.??????' | grep -q .; then
    fail "temporary state file was left behind"
fi

echo 'PASS: Strategy Lab cancellation is atomic, persistent, localized, and idempotent'
