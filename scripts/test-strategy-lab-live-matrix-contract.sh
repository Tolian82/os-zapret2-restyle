...
CURRENT_TIMEOUT_EVIDENCE="${ROOT_DIR}/docs/verification/evidence/2026-08-09-v0.4.0_6-stage60-timeout.md"
...
# change-specific PASS without promoting unrelated pending rows. Later timeout-hierarchy
# owner evidence may select corrective source work without retroactively changing those
# release-selected/focused PASS records.
if grep -Fq 'Overall status: **RELEASE-SELECTED LIVE GATE PASS ON `_27`; ADAPTIVE `_28` FOCUSED PASS; FULL REGRESSION MATRIX OPEN**' "${MATRIX}"; then
    for file in "${CURRENT_RELEASE_EVIDENCE}" "${CURRENT_ADAPTIVE_EVIDENCE}" "${CURRENT_TIMEOUT_EVIDENCE}" "${LIVE_GATE_DECISION}"
    do
        [ -s "${file}" ] || { echo "FAIL: current release/live evidence record is missing: ${file}" >&2; exit 1; }
    done
    ...
    grep -Fq 'Latest owner-tested diagnostic job: `job.Y8bR9M`' "${MATRIX}"
    ...
    grep -Fq 'Stage-50 adapter duration: about `39.065 s`' "${CURRENT_TIMEOUT_EVIDENCE}"
    grep -Fq 'Stage-60 adapter duration was approximately `70.07 s`' "${CURRENT_TIMEOUT_EVIDENCE}"
    grep -Fq 'no temporary Strategy Lab rule in `19100-19131` remained' "${CURRENT_TIMEOUT_EVIDENCE}"
    ...
    echo 'PASS: _27 remains the v0.4.0 release row, _28 keeps focused owner evidence, _6 timeout-hierarchy evidence selects the current source candidate, and rows 2-18 remain regression backlog'
    exit 0
fi
