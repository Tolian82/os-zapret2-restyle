...
TEST_SCRIPT="${ROOT_DIR}/src/opnsense/scripts/OPNsense/Zapret/test_domain.sh"; CONTROLLER="${ROOT_DIR}/src/opnsense/mvc/app/controllers/OPNsense/Zapret/Api/DiagnosticsController.php"
TMP_ROOT=$(mktemp -d); trap 'rm -rf "${TMP_ROOT}"' EXIT HUP INT TERM; MOCK_BIN="${TMP_ROOT}/bin"; mkdir -p "${MOCK_BIN}"
...
MOCK
chmod +x "${MOCK_BIN}/"*
run_case(){ set +e; output=$(MOCK_CURL_EXIT="$1" PATH="${MOCK_BIN}:${PATH}" sh "${TEST_SCRIPT}" example.com 2>&1); status=$?; set -e; [ "${status}" -eq 0 ]; printf '%s\n' "${output}" | grep -Fq "$2"; }
