...
run_connectivity_case() {
    curl_status="$1"; expected_result="$2"
    set +e
    output=$(MOCK_CURL_EXIT="${curl_status}" PATH="${MOCK_BIN}:${PATH}" sh "${TEST_SCRIPT}" example.com 2>&1)
    status=$?
    set -e
    [ "${status}" -eq 0 ] || exit 1
...
sh "${ROOT_DIR}/scripts/test-strategy-lab-parameter-expansion.sh"
sh "${ROOT_DIR}/scripts/test-strategy-lab-stability-shortlist.sh"
