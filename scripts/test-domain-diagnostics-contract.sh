...
TMP_ROOT=$(mktemp -d); trap 'rm -rf "${TMP_ROOT}"' EXIT HUP INT TERM
...
set +e; invalid=$(PATH="${MOCK_BIN}:${PATH}" sh "${TEST_SCRIPT}" 'bad/domain' 2>&1); code=$?; set -e
...
for test in job-contract candidate-runtime family-screening parameter-expansion stability-shortlist extended-tcp
