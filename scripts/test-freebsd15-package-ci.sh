...
for file in \
    ...
    "${MODEL_C_LIVE}" "${MODEL_C_DECISION}" "${MODEL_B_DECISION}"
do
...
# One canonical matrix discovers retained and new Strategy Lab focused contracts.
...
# Accepted Stage-60 architecture remains C -> B -> cold A; `_25` changes only concrete
# controlled source-port ownership and leaves the exact attribution boundary intact.
...
require "${PYTHON_ENTRY}" 'with stage60_source_port_lease.install()'

# `_25` exact free-port lease contract.
require "${SOURCE_PORT_LEASE_PY}" 'source-port-free'
require "${SOURCE_PORT_LEASE_PY}" 'preferred-free-else-alternate'
require "${SOURCE_PORT_LEASE_PY}" 'foreign_port_action'
require "${SOURCE_PORT_LEASE_PY}" 'skip-only'
require "${SOURCE_PORT_LEASE_PY}" 'original_model_c_batch'
require "${SOURCE_PORT_LEASE_PY}" 'original_model_b_batch'
require "${SOURCE_PORT_LEASE_PY}" 'Stage-60 controlled source-port alternate pool is exhausted'
...
# Current Engineering Memory retains accepted history and the exact `_23` live input for `_25`.
...
require "${MODEL_C_LIVE}" 'semantic restoration is therefore exact `RUNNING -> RUNNING`'
...
require "${SOURCE_PORT_PATCH}" 'number of endpoints × IPv4/IPv6 × TLS/QUIC × Generic UDP × Standard/Extended mode'
require "${PROJECT_STATE}" 'Current source line: `VERSION=0.4.0`, `PLUGIN_REVISION=25`'
require "${PROJECT_STATE}" 'fresh independent lease'
require "${PROJECT_STATE}" 'number of endpoints × IPv4/IPv6 × TLS/QUIC × Generic UDP × Standard/Extended mode'
...
sh -n "$0"
printf '%s\n' "PASS: FreeBSD 15 package CI preserves accepted Strategy Lab history while ${candidate} packages Model C with exact free-port leasing and Model-B/Model-A fail-closed fallback"
