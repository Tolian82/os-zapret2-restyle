...
# The Model A regression is part of the canonical corrective matrix on Linux; the
# FreeBSD job then builds the complete OPNsense tree into the FreeBSD:15 package. This
# measurement/correction slice does not require a separate VM execution contract before
# live owner measurement; its FreeBSD-specific system commands are optional identity probes.
...
grep -Fq 'Latest published testing candidate: `os-zapret2-restyle-0.4.0_10.pkg`' "${MATRIX}" ||
    fail 'live matrix does not select the published _10 candidate'
grep -Fq 'Latest owner-tested candidate: `os-zapret2-restyle-0.4.0_10.pkg`' "${MATRIX}" ||
    fail 'live matrix does not select the owner-tested _10 candidate'
grep -Fq "Current source candidate: \`${candidate}\`" "${MATRIX}" ||
    fail 'live matrix does not select the current Model A RSS corrective source candidate'
grep -Fq 'CURRENT MODEL A RSS CORRECTION — `v0.4.0_11`' "${MATRIX}" ||
    fail 'live matrix does not expose the Model A RSS correction handoff'
grep -Fq 'Latest owner-tested Model A job: `job.Oeq7Rc` (`rutracker.org`)' "${MATRIX}" ||
    fail 'live matrix does not retain the first Model A appliance job'
grep -Fq 'Required package ABI: `FreeBSD:15:amd64`' "${MATRIX}" ||
    fail 'live matrix does not require the FreeBSD 15 ABI'

grep -Fq 'Latest published testing prerelease: `v0.4.0_10` / `os-zapret2-restyle-0.4.0_10.pkg`' "${PROJECT_STATE}" ||
    fail 'project state does not select the published _10 candidate'
grep -Fq 'Latest owner-tested testing candidate: `v0.4.0_10` / `os-zapret2-restyle-0.4.0_10.pkg`' "${PROJECT_STATE}" ||
    fail 'project state does not select the owner-tested _10 candidate'
grep -Fq "Current source candidate: \`${candidate}\`" "${PROJECT_STATE}" ||
    fail 'project state does not select the current Model A RSS corrective source candidate'
...
grep -Fq 'Current phase: **Model A cold-reference RSS propagation correction and recheck**' "${PROJECT_STATE}" ||
    fail 'project state does not select the Model A RSS corrective phase'

sh -n "$0"
printf '%s\n' "PASS: FreeBSD 15 package CI, Python 3.13 Strategy Lab layers, _32 containment, _33 owner-live adaptive validation, _10 Model A baseline, and ${candidate} RSS correction qualification are synchronized"
