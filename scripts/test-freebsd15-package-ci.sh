...
# FreeBSD job then builds the complete OPNsense tree into the FreeBSD:15 package. The
# accepted appliance reference is documented separately and does not change packaging.
...
grep -Fq 'Overall status: **RELEASE-SELECTED LIVE GATE PASS ON `_27`; ADAPTIVE `_28` FOCUSED PASS; `_32` TIMEOUT-CONTAINMENT LIVE PASS; `_33` ADAPTIVE-VALIDATION CHANGE-SPECIFIC LIVE PASS; MODEL A COLD REFERENCE COLLECTED ON `_11`; FULL REGRESSION MATRIX OPEN**' "${MATRIX}" ||
...
grep -Fq "Current source candidate: \`${candidate}\`" "${MATRIX}" ||
    fail 'live matrix does not select the accepted Model A source candidate'
...
grep -Fq 'Latest owner-tested Model A job: `job.TtZeaH` (`rutracker.org`)' "${MATRIX}" ||
    fail 'live matrix does not select the accepted Model A appliance job'
...
grep -Fq 'Latest published testing prerelease: `v0.4.0_11` / `os-zapret2-restyle-0.4.0_11.pkg`' "${PROJECT_STATE}" ||
    fail 'project state does not select the published _11 candidate'
grep -Fq 'Latest owner-tested testing candidate: `v0.4.0_11` / `os-zapret2-restyle-0.4.0_11.pkg`' "${PROJECT_STATE}" ||
    fail 'project state does not select the owner-tested _11 candidate'
grep -Fq "Current source candidate: \`${candidate}\`" "${PROJECT_STATE}" ||
    fail 'project state does not select the accepted Model A source candidate'
...
grep -Fq 'Model A experiment gate: **REFERENCE COLLECTED on `v0.4.0_11` / `job.TtZeaH`**' "${PROJECT_STATE}" ||
    fail 'project state does not close the Model A reference gate'
grep -Fq 'Current phase: **Model A cold reference collected; Model B warm-worker coexistence experiment is next**' "${PROJECT_STATE}" ||
    fail 'project state does not select the Model B experiment handoff'

sh -n "$0"
printf '%s\n' "PASS: FreeBSD 15 package CI, Python 3.13 Strategy Lab layers, _32 containment, _33 owner-live adaptive validation, and ${candidate} accepted Model A cold reference are synchronized"
